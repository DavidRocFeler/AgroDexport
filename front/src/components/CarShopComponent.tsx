...
const PayPalButtonsComponent: React.FC<PayPalComponentProps> = ({
  amount,
  companyId,
  onSuccess,
  onError,
  ...
  return (
    ...
    <div className="w-4/5 p-6 bg-white rounded-lg h-4/5">
      <PayPalButtons
...
const CarShopComponent: React.FC<ILabelComponentProps> = ({ products }) => {
  ...
  const [selectedProduct, setSelectedProduct] = useState<IAgriProduct | null>(
    null
  );
  ...
  const handleRemoveProduct = (productId: string) => {
    setProductList((prevProducts) => {
      const updatedProducts = prevProducts.filter(
        (product) => product.company_product_id !== productId
      );
  ...
  const calculateTotals = () => {
    if (
      !selectedProduct ||
      !selectedProduct.company_price_x_kg ||
    ...
    const subtotal =
      selectedProduct.company_price_x_kg * (selectedProduct.quantity * 1000);
  ...
  return (
    <div className="flex flex-row pl-[2rem] pr-[2rem] pt-[3rem] pb-[2rem]  h-[85vh]">
      <div className="w-[60%]">
