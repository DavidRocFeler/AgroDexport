...
const UserList: React.FC = () => {
    ...
    useEffect(() => {
        if (session?.user) {
            // Añadir el usuario de la sesión al estado global si no existe
            const sessionUser: IUser = {
                id: session.user.id || Date.now().toString(),
                name: session.user.name || "",
                email: session.user.email || "",
                role: null, // Usamos 'authenticated' como rol por defecto
                // Añade otros campos necesarios según tu interfaz IUser
                lastname: "", // Asumiendo que estos campos son requeridos por IUser
                password: "",
                confirmPassword: "",
                legalAge: true,
            };
    ...
    useEffect(() => {
        console.log("Users in global state:", users); // Log de los usuarios
    }, [users]);
