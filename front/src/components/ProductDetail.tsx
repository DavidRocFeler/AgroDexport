...
const ProductDetail: React.FC = () => {
  const [product, setProduct] = useState<IAgriProduct | null>(null);
  const router = useRouter();

  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const filters = {}; // Aquí puedes agregar filtros si los necesitas
        const products = await getProductDB(filters);
        if (products.length > 0) {
          setProduct(products[0]); // Puedes modificar esto para obtener un producto específico
        }
      } catch (error) {
  ...
  const handleAddShop = () => {
    if (product) {
      ...
      const existingProductIndex = existingCart.findIndex(
        (p) => p.company_product_id === product.company_product_id
      );
    ...
    <div className="md:flex flex-col pt-[2rem] pb-[2rem] ">
      <div className="flex flex-row justify-evenly">
        <div className="flex flex-col w-[40%] border-[1px] border-black border-solid ">
          <figure className={styles.MainPicture}>
            <img
              src={product.company_product_img}
              alt="MainProduct"
              className="w-[30rem] m-auto object-cover "
            />
          </figure>
        ...
        <div className="border-[1px] border-black border-solid w-[45%] relative overflow-hidden">
          <table className={styles.CustomTable}>
            <tbody>
              ...
              {/* Más filas con los detalles del producto */}
            </tbody>
          </table>
          <div className="border-black border-solid border-[1px] m-auto absolute bottom-0 ">
            <p className="p-[2rem]">{product.company_product_description}</p>
          </div>
        </div>
      </div>
      <div className="flex flex-row justify-evenly mt-[2rem]">
